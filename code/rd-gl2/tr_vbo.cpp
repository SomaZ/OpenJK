...
VBO_t *R_CreateVBO(byte * vertexes, int vertexesSize, vboUsage_t usage)
{
	...
	vbo = tr.vbos[tr.numVBOs] = (VBO_t *)R_Hunk_Alloc(sizeof(*vbo), qtrue);
...
void RB_BindUniformBlock(uniformBlock_t block)
{
	...
	qglBindBufferRange(GL_UNIFORM_BUFFER, blockInfo->slot, thisFrame->ubo, thisFrame->uboWriteOffset, blockInfo->size);
}
