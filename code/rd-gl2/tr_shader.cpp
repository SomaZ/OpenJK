...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		//
		// parallaxBias <value> continue, rend2 only
		//
		...
		// If this stage is cloth... continue... rend2 only
...
static qboolean CollapseStagesToGLSL(void)
{
	...
	if (!skip)
	{
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++)
		{
			...
			vertexlit = qfalse;
			if (diffuse->rgbGen == CGEN_VERTEX_LIT || diffuse->rgbGen == CGEN_EXACT_VERTEX_LIT || diffuse->rgbGen == CGEN_EXACT_VERTEX || diffuse->rgbGen == CGEN_VERTEX)
			{
				vertexlit = qtrue;
			}
...
static void CreateExternalShaders( void ) {
	...
	{
		ri.Printf(PRINT_ALL, "Sun flare shader is missing. Generating from flare or dlight image\n");
