...
public:
	...
	{
		bool ok=re.G2ABI_TestModelPointers(ghlInfo);
		if (!ok)
		{
...
public:
	...
	void TestAllAnims()
	{
		for (size_t j=0;j<MAX_G2_MODELS;j++)
		{
			...
			{
				if (re.G2ABI_SetupModelPointers(&ghoul2[i]))
				{
						  ...
						  qhandle_t customShader, int modelFlags, int lodBias)
{
	...
	if (model==ghoul2.size())
	{
		...
		info.mModelindex = 0;
		if(re.G2ABI_TestModelPointers(&info)) {
	...
	if (!re.G2ABI_TestModelPointers(&ghoul2[model]))
	{
...
qboolean G2API_SetLodBias(CGhoul2Info *ghlInfo, int lodBias)
{
	...
	if (re.G2ABI_SetupModelPointers(ghlInfo))
	{
...
qboolean G2API_SetSkin(CGhoul2Info *ghlInfo, qhandle_t customSkin, qhandle_t renderSkin)
{
	...
	if (re.G2ABI_SetupModelPointers(ghlInfo))
	{
		...
		{//this is going to set the surfs on/off matching the skin file
			re.G2ABI_SetSurfaceOnOffFromSkin( ghlInfo, renderSkin );
		}
		return qtrue;
	}
#endif
...
qboolean G2API_SetBoneAnim(CGhoul2Info *ghlInfo, const char *boneName, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int AcurrentTime, const float setFrame, const int blendTime)
{
	...
	if (boneName&&re.G2ABI_SetupModelPointers(ghlInfo))
	{
		G2ERROR(startFrame>=0,"startframe<0");
		G2ERROR(startFrame<ghlInfo->aHeader->numFrames,"startframe>=numframes");
		G2ERROR(endFrame>0,"endframe<=0");
...
{
	qboolean ret=qfalse;
	if (re.G2ABI_SetupModelPointers(ghlInfo)&&re.G2ABI_SetupModelPointers(ghlInfoTo))
	{
...
qboolean G2API_AttachEnt(int *boltInfo, CGhoul2Info *ghlInfoTo, int toBoltIndex, int entNum, int toModelNum)
{
	...
	if (boltInfo&&re.G2ABI_SetupModelPointers(ghlInfoTo))
	{
		// make sure we have a model to attach, a model to attach to, and a bolt on that model
...
{
	mdxmHeader_t *mdxm = re.G2ABI_GetMdxmByHandle(modelIndex);
...
{
	mdxaHeader_t *mdxa = re.G2ABI_GetMdxaByHandle(modelIndex);
...
char *G2API_GetSurfaceName(CGhoul2Info *ghlInfo, int surfNumber)
{
	...
	if (re.G2ABI_SetupModelPointers(ghlInfo))
	{
		...
		if (surf)
		{
			...
			mdxmHeader_t *mdxm = re.G2ABI_GetMdxmByModel(ghlInfo->currentModel);
...
int	G2API_GetSurfaceIndex(CGhoul2Info *ghlInfo, const char *surfaceName)
{
	...
	if (surfaceName&&re.G2ABI_SetupModelPointers(ghlInfo))
	{
...
qboolean G2_SetupModelPointers(CGhoul2Info_v &ghoul2) // returns true if any model is properly set up
{
	...
	for (i=0; i<ghoul2.size(); i++)
	{
		qboolean r=re.G2ABI_SetupModelPointers(&ghoul2[i]);
		ret=(qboolean)(ret||r);
	}
	return ret;
}
