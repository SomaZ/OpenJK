...
// Given a bone name, see if that bone is already in our bone list - note the model_s pointer that gets passed in here MUST point at the
...
int G2_Add_Bone (const model_s *mod, boneInfo_v &blist, const char *boneName)
{
	...
	mdxaHeader_t *mdxa = re.G2ABI_GetMdxaByModel(mod);
