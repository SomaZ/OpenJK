...
CGoreSet::~CGoreSet()
{
	...
	{
		re.DeleteGoreRecord((*i).second.mGoreTag);
	}
};
#endif
...
{
	int			i, x;
	mdxmHeader_t *mdxm = re.G2ABI_GetMdxmByHandle(re.RegisterModel(fileName));
...
void G2_List_Model_Bones(const char *fileName, int frame)
{
	...
	mdxmHeader_t *mdxm = re.G2ABI_GetMdxmByHandle(re.RegisterModel(fileName));
	mdxaHeader_t *mdxa = re.G2ABI_GetMdxaByHandle(mdxm->animIndex);
...
#endif
{
	...
	for (i=0; i<ghoul2.size(); i++)
	{
		...
		if (ApplyGore)
		{
			...
			if (lod>=re.G2ABI_GetNumLods(g.currentModel))
			{
		...
		mdxmHeader_t *mdxm = re.G2ABI_GetMdxmByModel(g.currentModel);
...
static void G2_TraceSurfaces(CTraceSurface &TS)
{
	...
	mdxmHeader_t *mdxm = re.G2ABI_GetMdxmByModel(TS.currentModel);
	assert(mdxm);
...
{
	assert(mod);
	mdxmHeader_t *mdxm = re.G2ABI_GetMdxmByModel(mod);
	assert(mdxm);
