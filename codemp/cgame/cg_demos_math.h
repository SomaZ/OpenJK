...
static QINLINE float QuatDot( const Quat_t q0, const Quat_t q1) {
	return q0[0]*q1[0] + q0[1]*q1[1] + q0[2]*q1[2] + q0[3]*q1[3];
}
static QINLINE void QuatCopy( const Quat_t src, Quat_t dst) {
	dst[0] = src[0];dst[1] = src[1];dst[2] = src[2];dst[3] = src[3];
}
static QINLINE void QuatNegate( const Quat_t src, Quat_t dst) {
	dst[0] = -src[0];dst[1] = -src[1];dst[2] = -src[2];dst[3] = -src[3];
}
static QINLINE void QuatConjugate( const Quat_t src, Quat_t dst) {
	dst[0] = -src[0];dst[1] = -src[1];dst[2] = -src[2];dst[3] = src[3];
}
static QINLINE void QuatAdd( const Quat_t q0, const Quat_t q1, Quat_t qr) {
	qr[0] = q0[0] + q1[0];
	qr[1] = q0[1] + q1[1];
	qr[2] = q0[2] + q1[2];
	qr[3] = q0[3] + q1[3];
}
static QINLINE void QuatSub( const Quat_t q0, const Quat_t q1, Quat_t qr) {
	qr[0] = q0[0] - q1[0];
	qr[1] = q0[1] - q1[1];
	qr[2] = q0[2] - q1[2];
	qr[3] = q0[3] - q1[3];
}
static QINLINE void QuatScale( const Quat_t q0, float scale,Quat_t qr) {
	qr[0] = scale * q0[0];
	qr[1] = scale * q0[1];
	qr[2] = scale * q0[2];
	qr[3] = scale * q0[3];
}
static QINLINE void QuatScaleAdd( const Quat_t q0, const Quat_t q1, float scale, Quat_t qr) {
	qr[0] = q0[0] + scale * q1[0];
	qr[1] = q0[1] + scale * q1[1];
	qr[2] = q0[2] + scale * q1[2];
	qr[3] = q0[3] + scale * q1[3];
}
static QINLINE float QuatDistanceSquared( const Quat_t q1, const Quat_t q2 ) {
	Quat_t q;
	QuatSub(q1, q2, q);
	return q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3];
}
static QINLINE float QuatLengthSquared( const Quat_t q ) {
	return q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3];
}
static QINLINE float QuatLength( const Quat_t q ) {
	return sqrt( q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3]);
}
static QINLINE void QuatClear( Quat_t q ) {
	q[0] = q[1] = q[2] = q[3] = 0;
}

static QINLINE float QuatNormalize( Quat_t q ) {
...
static QINLINE void posMatrix( float t, posInterpolate_t type, vec4_t matrix) {
	switch ( type ) {
	case posLinear:
		matrix[0] = 0;
		matrix[1] = 1-t;
...
static QINLINE void posGet( float t, posInterpolate_t type, const vec3_t control[4], vec3_t out) {
	vec4_t matrix;
...
static QINLINE vec_t VectorDistanceSquared ( const vec3_t v1, const vec3_t v2 ) {
	vec3_t v;
	VectorSubtract( v1, v2, v );
	return (v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
}
static QINLINE vec_t VectorDistance( const vec3_t v1, const vec3_t v2 ) {
	vec3_t v;
	VectorSubtract( v1, v2, v );
	return (vec_t)sqrt (v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
}
static QINLINE void AnglesNormalize360( vec3_t angles) {
	angles[0] = AngleNormalize360(angles[0]);
	angles[1] = AngleNormalize360(angles[1]);
	angles[2] = AngleNormalize360(angles[2]);
}
static QINLINE void AnglesNormalize180( vec3_t angles) {
...
static QINLINE int fxRand( int r ) {
	r = (69069 * r + 1);
	return r;
}
static QINLINE float fxRandom( int r ) {
	return (fxRand(r) & 0xffff) * (1.0f / 0x10000);
}
static QINLINE int fxRandomSign( int r ) {
	float sign;
	sign = fxRandom(r);
