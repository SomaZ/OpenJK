...
void S_MuteSound( int entityNum, int entchannel ) {
	S_DMAStopSound(entityNum, entchannel, -1);
	S_MMEStopSound(entityNum, entchannel, -1);
}
...
void S_StartSound( const vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle ) {
	S_StartSound(origin, entityNum, entchannel, 127, sfxHandle);
}
...
void S_AddLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfxHandle ) {
	S_AddLoopingSound(((char *)cl.entityBaselines) + entityNum, origin, velocity, sfxHandle, 127);
}
...
void S_StopLoopingSound( int entityNum ) {
	//nothing since S_AddLoopingSound is called on every frame to keep going
}
...
void S_StartBackgroundTrack( const char *intro, const char *loop, qboolean bReturnWithoutStarting ) {
	...
	if (!bReturnWithoutStarting && soundExists) {
