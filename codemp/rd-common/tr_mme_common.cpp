...
QINLINE byte * R_MME_BlurOverlapBuf(mmeBlurBlock_t *block) {
...
#if (!defined (HAVE_GLES) || defined (X86_OR_64)) && !defined (__arm64__) && !defined (WIN64)
	_mm_empty();
#endif
}

static void MME_AccumClear( void * QRESTRICT w, const void * QRESTRICT r, short mul, int count ) {
...
static void MME_AccumAdd( void * QRESTRICT w, const void * QRESTRICT r, short mul, int count ) {
	const byte (*reader)[8] = (const byte(*)[8])r;
	int32_t (*writer)[8] = (int32_t(*)[8])w;
	int32_t intmul = mul;
...
void R_MME_BlurAccumAdd( mmeBlurBlock_t *block, const void *add) {
	...
	if ( mme_cpuSSE2->integer ) {
		...
		if ( index == 0) {
			MME_AccumClear( block->accum, add, control->MMX[ index ], block->count );
		} else {
			MME_AccumAdd( block->accum, add, control->MMX[ index ], block->count );
		}
	}
#else
	if ( index == 0) {
...
qboolean R_MME_JitterTableMask(float *jitarr, int num, char *name) {
	...
	if (pic) {
		...
		int addedSamples = 0;
		int longerSide = Q_max(width, height);
