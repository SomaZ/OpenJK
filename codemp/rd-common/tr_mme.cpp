...
static QINLINE workData_t *work( qboolean stereo ) {
...
void R_MME_PrepareMultiCapture( const void *data ) {
	...
	while ( 1 ) {
		...
		case RC_SET_COLOR:
			data = (const void *)((const setColorCommand_t *)data + 1);
			break;
		case RC_STRETCH_PIC:
			data = (const void *)((const stretchPicCommand_t *)data + 1);
			break;
		case RC_ROTATE_PIC:
			data = (const void *)((const rotatePicCommand_t *)data + 1);
			break;
		case RC_ROTATE_PIC2:
			data = (const void *)((const rotatePicCommand_t *)data + 1);
			break;
		case RC_ROTATE_PIC2_RATIOFIX:
			data = (const void *)((const rotatePicRatioFixCommand_t *)data + 1);
			break;
		case RC_DRAW_SURFS:
			data = (const void *)((const drawSurfsCommand_t *)data + 1);
			break;
		case RC_DRAW_BUFFER:
			data = (const void *)((const drawBufferCommand_t *)data + 1);
			break;
		case RC_SWAP_BUFFERS:
			data = (const void *)((const swapBuffersCommand_t *)data + 1);
			break;
		case RC_SCREENSHOT:
			data = (const void *)((const screenShotCommand_t *)data + 1);
			break;
		case RC_SCISSOR:
			data = (const void *)((const scissorCommand_t *)data + 1);
			break;
		case RC_COLORMASK:
			data = (const void *)((const colorMaskCommand_t *)data + 1);
			break;
		case RC_CLEARDEPTH:
			data = (const void *)((const clearDepthCommand_t *)data + 1);
			break;
		case RC_CONVOLVECUBEMAP:
			data = (const void *)((const convolveCubemapCommand_t *)data + 1);
			break;
		case RC_POSTPROCESS:
			data = (const void *)((const postProcessCommand_t *)data + 1);
			break;
		case RC_BEGIN_TIMED_BLOCK:
			data = (const void *)((const beginTimedBlockCommand_t *)data + 1);
			break;
		case RC_END_TIMED_BLOCK:
			data = (const void *)((const endTimedBlockCommand_t *)data + 1);
			break;
		case RC_CAPTURE:
...
//			}
			...
			if ( mme_saveShot->integer == 1 ) {
				R_MME_MultiShot( outAlign, stereo );
				if ( doGamma && mme_blurGamma->integer ) {
...
void R_MME_Capture( const char *shotName, float fps, float focus, float radius ) {
	...
	if (mme_dofFrames->integer > 0 || r_stereoSeparation->value != 0.0f || mme_saveCubemap->integer > 0)
		tr.capturingMultiPass = qtrue;
	if (tr.capturingMultiPass)
		tr.firstMultiPassFrame = qtrue;
